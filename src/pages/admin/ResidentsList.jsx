...
async function fetchResidents({ q, flat, filter, page: pageArg }) {
  ...
  // Single effect keyed on both filter and page: changing the filter also
  // resets the page to 1, and this fires exactly once for that combined
  // change instead of once for the filter reset and again for the
  // resulting page change.
  useEffect(() => {
    setLoading(true);
    setError('');
