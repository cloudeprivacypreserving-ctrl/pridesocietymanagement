// Password show/hide toggle rendered as a little monkey that covers its
// eyes while the password is hidden and drops its hands to peek when it's
// revealed. Ported from Milan Raring's "Show & Hide Password" CodePen
// (https://codepen.io/milanraring/pen/eYYqjeV) — pure inline SVG + CSS,
// no dependencies or image assets.
//
// Replaces the plain eye-icon button in the password field. It's a real
// <button type="button"> so keyboard/AT users get the same control, with
// the same aria-label the eye icon had.
export default function MonkeyPasswordToggle({ eyesCovered, onToggle, showPassword }) {
  return (
    <button
      type="button"
      className={`monkey-toggle${eyesCovered ? '' : ' is-open'}`}
      onClick={onToggle}
      aria-label={showPassword ? 'Hide password' : 'Show password'}
      aria-pressed={showPassword}
    >
      <span className="monkey-hands" aria-hidden="true">
      ...
      <span className="monkey-face" aria-hidden="true">
        <svg viewBox="0 0 64 64">
          ...
          <path
            ...
            fill="#e0ac7e"
          />
          <g fill="#3b302a" className="monkey-eyes">
